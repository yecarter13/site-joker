...
import { HiPlus, HiX, HiPhotograph, HiLocationMarker, HiChevronDown, HiChevronUp } from "react-icons/hi";
...
export default function PropertyForm({ initial }: PropertyFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div>
        <h3 className="text-lg font-bold text-gray-900 mb-4">Informations principales</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-1">
            ...
            <select name="type" defaultValue={initial?.type || "Privé"} className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none bg-white">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1">
            ...
            <input name="city" defaultValue={initial?.city} required className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="Ex: Paris" />
          </div>
          <div className="space-y-1">
            <label className="block text-sm font-medium text-gray-700">Quartier</label>
            <input name="district" defaultValue={initial?.district || ""} className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="Ex: 11e arrondissement" />
          </div>
          <div className="space-y-1 md:col-span-2">
            <label className="block text-sm font-medium text-gray-700">Adresse</label>
            <input name="address" defaultValue={initial?.address || ""} className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="Ex: 123 Rue de la Paix" />
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="space-y-1">
          <textarea name="description" defaultValue={initial?.description} required rows={5} className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="Description détaillée du logement..." />
        </div>
      </div>
          ...
          <div className="flex flex-wrap gap-3">
            {images.map((url, i) => (
              <div key={url} className="relative w-28 h-28 rounded-xl overflow-hidden group border border-gray-200">
                ...
                {i > 0 && (
                  <button type="button" onClick={() => moveImage(i, -1)} className="absolute top-1 left-1 bg-white/80 text-gray-600 rounded-full p-0.5 opacity-0 group-hover:opacity-100 transition-opacity text-xs">
                    ←
                  </button>
                )}
                {i < images.length - 1 && (
                  <button type="button" onClick={() => moveImage(i, 1)} className="absolute top-1 left-7 bg-white/80 text-gray-600 rounded-full p-0.5 opacity-0 group-hover:opacity-100 transition-opacity text-xs">
                    →
                  </button>
                )}
                <button type="button" onClick={() => removeImage(url)} className="absolute top-1 right-1 bg-red-500 text-white rounded-full p-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                  <HiX size={14} />
                </button>
            ...
            <button type="button" onClick={() => fileRef.current?.click()} disabled={uploading} className="w-28 h-28 border-2 border-dashed border-gray-300 rounded-xl flex items-center justify-center text-gray-400 hover:text-indigo-500 hover:border-indigo-500 transition-colors bg-gray-50 hover:bg-indigo-50">
              {uploading ? <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-indigo-500" /> : <HiPlus size={24} />}
            </button>
          </div>
          <input ref={fileRef} type="file" accept="image/*" onChange={handleUpload} className="hidden" />
          <p className="text-xs text-gray-400">La première photo sera utilisée comme image principale sur les cartes. Cliquez ← → pour réorganiser.</p>
        </div>
      </div>

      {/* Détails supplémentaires (toggle) */}
      <div>
        <button type="button" onClick={() => setShowExtra(!showExtra)} className="flex items-center gap-2 text-sm font-semibold text-gray-600 hover:text-gray-900 transition-colors">
        ...
        {showExtra && (
          <div className="mt-4 space-y-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="space-y-1">
                <label className="block text-sm font-medium text-gray-700">Année construction</label>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="space-y-1">
                ...
                <select name="heating" defaultValue={initial?.heating || ""} className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none bg-white">
                  <option value="">Non spécifié</option>
                  <option value="Individuel électrique">Individuel électrique</option>
              ...
              <div className="space-y-1">
                <label className="block text-sm font-medium text-gray-700">Meublé</label>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Équipements et services</label>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="space-y-1 md:col-span-3">
            <label className="block text-xs text-gray-500">Lien Google Maps (iframe)</label>
            <input name="mapLink" defaultValue={initial?.mapLink || ""} className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="Lien Google Maps" />
          </div>
          <div className="space-y-1">
            <label className="block text-xs text-gray-500">Latitude</label>
            <input name="latitude" type="number" step="any" defaultValue={initial?.latitude ?? ""} className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="48.8566" />
          </div>
          <div className="space-y-1">
            <label className="block text-xs text-gray-500">Longitude</label>
            <input name="longitude" type="number" step="any" defaultValue={initial?.longitude ?? ""} className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="2.3522" />
          </div>
        </div>
      </div>

      <div className="flex gap-3 pt-4 border-t border-gray-200">
        <button type="submit" disabled={saving} className="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3 rounded-xl font-semibold text-sm transition-all disabled:opacity-50 shadow-lg">
          {saving ? "Enregistrement..." : initial ? "Mettre à jour" : "Ajouter le logement"}
        </button>
        <button type="button" onClick={() => router.back()} className="bg-gray-100 hover:bg-gray-200 text-gray-700 px-8 py-3 rounded-xl font-semibold text-sm transition-all">
          Annuler
        </button>
      </div>
    </form>
